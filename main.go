...
func (r Running) Calories() float64 {
	cal := ((CaloriesMeanSpeedMultiplier*r.meanSpeed() + CaloriesMeanSpeedShift) * r.Weight / MInKm * float64(r.Duration.Hours()) * MinInHours)
	return cal
}
...
func (w Walking) Calories() float64 {
	speedInM := w.meanSpeed() * KmHInMsec // скорость в метрах в секунду
	cal := ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedInM, 2)/w.Height/CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * float64(w.Duration.Hours()) * MinInHours)
	return cal
}
...
func (s Swimming) Calories() float64 {
	var cal = (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * float64(s.Duration.Hours())
	return cal
}
